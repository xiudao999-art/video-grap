...
用法：
  ...
  # 手动登录导出登录态
  $PY scripts/browser_unlock.py login --url "https://target.com/login" --out storage_state.json
  # 持久 profile(更稳, 适合 sessionStorage 站)
  $PY scripts/browser_unlock.py login --url "https://target.com/login" --user-data-dir .hybrid_profile --out storage_state.json
  # LLM 自动登录(需 ANTHROPIC_API_KEY 或 OPENAI_API_KEY)
  $PY scripts/browser_unlock.py login --url "https://target.com" --task "用账号X密码Y登录并进入订单页" --out storage_state.json
  # 只启动 Chrome 暴露 CDP(给 crawl_pipeline --cdp 共享同一浏览器)
  $PY scripts/browser_unlock.py cdp --url "https://target.com" --keep
"""
...
try:
    ...
    sys.exit("ERROR: 未装 browser-use。 cloak-test/.venv/Scripts/python.exe -m pip install browser-use")
...
async def cmd_login(args):
    ...
    # 导出 storage_state
    state = await browser.export_storage_state(args.out)
    n_cookies = len(state.get("cookies", [])) if isinstance(state, dict) else "?"
    print(f"[login] ✓ 已导出 storage_state → {args.out}  (cookies: {n_cookies})")
...
def main():
    ...
    p_login.add_argument("--out", default="storage_state.json", help="storage_state 输出路径")
